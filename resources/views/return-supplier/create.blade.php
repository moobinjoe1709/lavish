...
<div class="content-wrapper">
	...
	<div class="page-header page-header-default">
		<div class="page-header-content">
			<div class="page-title">
				<h4><i class="icon-arrow-left52 position-left"></i> <span class="text-semibold">ส่งคืนสินค้า (Lavish กับ Supplier)</span></h4>
			</div>
		...
		<div class="breadcrumb-line">
			<ul class="breadcrumb">
				...
				<li class="active">ส่งคืนสินค้า (Lavish กับ Supplier) / เพิ่ม </li>
			</ul>
		</div>
	</div>
	<!-- /page header -->
	...
	<div class="content">
		...
		<form id="myForm" method="post" action="{{url('returnsupplier_create')}}">
			{{ csrf_field() }}
			<div class="row">
				<div class="col-md-12">
					<!-- Vertical form -->
										...
										<div class="row">
											<div class="col-md-6">
												<div class="form-group">
													...
													<input type="text" class="form-control" name="customername" id="customername" placeholder="ชื่อซัพพลายเออร์" autocomplete="off" value="">
												</div>
											</div>
	...
	$('#search').click(function(){
		...
		'data': {
			...
			'success': function (data) {
				var num = 0;
				$('#customerid').val(data.import.import_supplierid);
				$('#customername').val(data.import.import_suppliername);
				$.each(data.orders,function(key,item){
					...
					if(find == 0){
						$('#rowdata').append('<tr class="rowbody" id="added_item'+item.sub_product+'">'
							+'<td align="center">'+item.product_code+'<input type="hidden" class="form-control" name="productid[]" id="productid'+item.sub_product+'" value="'+item.sub_product+'"></td>'
							+'<td align="left">'+item.product_name+'</td>'
							+'<td align="right"><input type="text" class="form-control" name="price[]" id="price'+item.sub_product+'" value="'+item.sub_price+'" onkeyup="changeprice('+item.sub_product+')" style="width:120px;" onkeypress="return isNumberKey(event);"></td>'
							+'<td align="center"><input type="text" class="form-control" name="qty[]" id="qty'+item.sub_product+'" value="'+item.sub_qty+'" onkeyup="changeqty('+item.sub_product+')" style="width:120px;" onkeypress="return isNumberKey(event);"></td>'
							+'<td align="right"><span id="totalprosp'+item.sub_product+'">'+formatNumber(item.sub_total.toFixed(2))+'</span><input type="hidden" class="form-control" name="total[]" id="total'+item.sub_product+'" value="'+parseFloat(item.sub_total)+'"></td>'
							+'<td align="center"><button type="button" class="btn btn-danger btn-icon" onclick="delrow('+item.sub_product+')"><i class="icon-minus-circle2"></i></button></td>'
						+'</tr>');
					}else{
						var qty = $('#qty'+item.sub_product).val();
						var sum = parseInt(qty)+1;
						var price = $('#price'+item.sub_product).val()||0;
						var totalpro = parseFloat(price*sum);
						
						$('#qty'+item.sub_product).val(sum);
						$('#totalprosp'+item.sub_product).text(formatNumber(totalpro.toFixed(2)));
						$('#total'+item.sub_product).val(totalpro);
					}
				});
