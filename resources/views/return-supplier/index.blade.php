	...
	<div class="page-header page-header-default">
		...
		<div class="breadcrumb-line">
			<ul class="breadcrumb">
				...
				<li class="active">ส่งคืนสินค้า (Lavish กับ Supplier)</li>
			</ul>
		</div>
	</div>
	<!-- /page header -->
				...
				<!-- Double border styling -->
					<div class="panel panel-flat">
						<div class="panel-heading">
							<h5 class="panel-title">ส่งคืนสินค้า</h5>
						...
						<div class="panel-body">
							<div class="pull-right">
								<a href="{{url('return/supplier/create')}}"><button type="button" class="btn btn-success btn-lg"><i class="icon-plus-circle2"></i> เพิ่ม</button></a>
							</div>
						</div>
...
<script>
	...
	$(document).ready(function(){
		var oTable = $('#datatables').DataTable({
			...
			lengthChange: false,
			ajax:{ 
				url : "{{url('returnsupplierdatatables')}}",
			},
			columns: [
			...
			rowCallback: function(row,data,index ){
				var status = '<span class="label bg-danger">คืนสินค้า</span>';
				$('td:eq(4)', row).html(status);
				$('td:eq(6)', row).html( '<i class="icon-trash" onclick="del('+data['export_id']+');" data-popup="tooltip" title="ลบรายการ"></i>' );
	...
	function del(id){
		bootbox.confirm({
			...
			callback: function (result){
				if(result == true){
					window.location.href="return/delete/"+id+"";
				}
			}
		});
	}
