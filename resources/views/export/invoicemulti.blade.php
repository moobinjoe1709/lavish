...
","มีนาคม","เมษายน","พฤษภาคม","มิถุนายน","กรกฎาคม","สิงหาคม","กันยายน","ตุลาคม","พฤศจิกายน","ธันวาคม");
			...
			if($data){
				foreach($data as $key => $rs){
					$exp 	= DB::table('export')->where('export_id',$rs)->first();
					$subexp = DB::table('orders')->join('product','product.product_id','=','orders.order_productid')->where('order_ref',$rs)->get();
									...
									<div style="border:1px solid #000; border-radius: 3px; width:100%; display:block; margin-bottom:15px;margin-top:7;">
										<div style="border-bottom:1px solid #000;">
											<div class="row">
												...
												<div class="col-xs-8">{{$exp->export_order}}</div>
											</div>
										</div>
										<div class="row">
											<div class="col-xs-2 text-center" style="line-height:12px; font-size:12px; font-weight: bold; border-right:1px solid #000; padding-top:3px; padding-bottom:3px;">เลขที่<br>Ref.No</div>
						...
						<div class="row">
							<div class="col-xs-12">
								<table style="width:100%;border: 1px solid black;">
										...
										@php
											...
											if($subexp){
												foreach($subexp as $ar){
													$totalpro += $ar->order_total;
													@endphp
													<tr>
														<td class="linedata" align="center">{{$num}}</td>
														<td class="linedata">{{$ar->product_code}}</td>
														<td class="linedata">{{$ar->product_name}}</td>
														<td class="linedata" align="center">{{$ar->order_qty}}</td>
														<td class="linedata" align="center">{{$ar->product_unit}}</td>
														<td class="linedata" align="right">{{number_format($ar->order_price,2)}}</td>
														<td class="linedata" align="right"></td>
														<td class="linedata" align="right"></td>
														<td class="linedata" align="right">{{number_format($ar->order_total,2)}}</td>
													</tr>
													@php
													$num++;
												}
									...
									<tfoot>
										<tr>
											<td align="left" style="padding-left:5px;border-right: 1px solid black;border-bottom: 1px solid black;" colspan="4"><span style="font-size:14px;">ตัวอักษร   :  {{num2wordsThai($exp->export_totalpayment)}}บาทถ้วน</span></td>
										...
										<tr>
											<td align="left" style="padding-left:5px;line-height:15px;" colspan="5" rowspan="3"><span style="font-size:12px;margin-top:10px;"><br>1. ได้รับสินค้าตามรายการข้างต้นในสภาพสมบูรณ์พร้อมต้นฉบับใบเสร็จรับเงินถูกต้องเรียบร้อยแล้ว<br>Received the above mentioned merchandise in goods order and condition with original receipt.<br>2.สินค้า/บริการตามใบส่งสินค้านี้ หากมีการชำรุดเสียหายหรือขาดตกบกพร่องประการใด โปรดแจ้งให้บริษัทฯ ทราบภายใน 7 วัน<br>และโปรดนำใบเสร็จรับเงินมาด้วยทุกครั้งหากไม่มีใบเสร็จรับเงินทางบริษัทฯขอสงวนสิทธิ์ในการเปลี่ยนสินค้า<br>Any Discrepancy reaiating to goods on this delivery order must be notified with in 7 days from date of purchase with the original receipt only. Otherwise claims will not be honored.</span></td>
