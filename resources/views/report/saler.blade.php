	...
	<div class="page-header page-header-default">
		<div class="page-header-content">
			<div class="page-title">
				<h4><i class="icon-arrow-left52 position-left"></i> <span class="text-semibold">รายงานเซลล์</span></h4>
			</div>
		...
		<div class="breadcrumb-line">
			<ul class="breadcrumb">
				...
				<li class="active">รายงานเซลล์</li>
			</ul>
		</div>
	</div>
	<!-- /page header -->
					...
					<div class="panel panel-flat">
						...
						<div class="panel-body">
							<div class="row">
								<div class="col-md-12">
									<fieldset>
										<legend class="text-semibold"><i class="icon-stack2"></i> รายงานเซลล์</legend>
										<form id="myForm" method="post" action="{{url('salerpdf')}}" target="_blank">
										{{ csrf_field() }}
										<div class="row">
											<div class="col-md-2">
												<label>เซลล์ :</label>
...
<script>
			...
			'success': function (data){
				...
				$.each(data,function(key,item){
					...
					text += '<tr class="rowbody">'
						...
						+'<td align="left">'+item.export_customername+'</td>'
						+'<td align="right">'+formatNumber(item.export_totalpayment.toFixed(2))+'</td>'
					+'</tr>';
					num++;
				});
				
				$('#rowdata').append(text);
				$('#rowfoot').append('<tr class="rowfoot"><td colspan="5" align="center">รวม</td><td align="right">'+formatNumber(total.toFixed(2))+'</td></tr>');
	...
	$('#excel').click(function(){
		...
		var saler 	= $('#saler').val();
		window.open('{{url("salerexcel")}}/'+starts+'/'+ends+'/'+saler);
	});
</script>
</html>
@stop